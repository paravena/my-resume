...
function checkDownloadSupport(): boolean {
  try {
    ...
    const hasRevokeObjectURL = typeof URL !== 'undefined' && 
...
export function checkBrowserSupport(): BrowserSupportResult {
  ...
  const isFullySupported = canvasSupported && blobSupported && downloadSupported;
 ...
 * ```
 */
export function getUnsupportedFeaturesMessage(support: BrowserSupportResult): string | null {
  if (support.isFullySupported) {
    return null;
  }
